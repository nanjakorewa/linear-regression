...
st.markdown(
    """
参考文献：[Linear regression diagnostics - statsmodels 0.15.0](https://www.statsmodels.org/dev/examples/notebooks/generated/linear_regression_diagnostics_plots.html)         
"""
)
...
st.markdown("## 線形回帰のフィッティング")
res = smf.ols(formula="y ~ x1 + x2 + x3", data=df).fit()
st.write(res.summary())
cls = LinearRegDiagnostic(res)

st.markdown("## 残差のプロット")
fig, ax = cls.residual_plot()
st.pyplot(fig)

st.markdown("## 正規Q-Qプロット")
fig, ax = cls.qq_plot()
st.pyplot(fig)

st.markdown("## 標準化した残差の絶対値の平方根のプロット")
fig, ax = cls.scale_location_plot()
st.pyplot(fig)

st.markdown("## Leverageに対する標準化した残差のプロット")
fig, ax = cls.leverage_plot(high_leverage_threshold=True, cooks_threshold="dof")
st.pyplot(fig)
